...
namespace RemoteAnalyst.UWSRelay.BLL {
    class CustomFTP {
        ...
        internal bool Connect(string sHost) {
            ...
            try {
                ...
                m_tcpClient.ReceiveTimeout = 180000; // wait 180 seconds before aborting
                m_tcpClient.SendTimeout = 180000; // wait 180 seconds before aborting
        ...
        internal bool Login(string sUsername, string sPassword) {
            if (!bConnected) return false;
        ...
        public bool Upload(string locFilename, string fileName, string remDir, string strFileCode, int pext, int sext, int mext, ILog log) {
            Socket dSocket;
            Stream fs;
            bool bSetDir = false;
            int BufferSize = 24576; //INCREASED "BufferSize" TO 24576 (24K) FROM 1024 (1K)
            int nBytes = 0;
            ...
            dSocket = CreateDataSocket();
            if (remDir != "Out") bSetDir = SetCurrentDirectory(remDir); //Set Current Directory.

            // create data connection with host
            string sReply = SendCommand("STOR " + fileName + "," + strFileCode + "," + pext + "," + sext + "," + mext);

            if (sReply[0] != '1') return false;

            byte[] bytes = new byte[BufferSize]; //INCREASED "BufferSize" TO 24576 (24K) FROM 1024 (1K)
            fs = File.Open(locFilename, FileMode.Open);
            while (true) {
                // read data and send it to host
                nBytes = fs.Read(bytes, 0, bytes.Length);

                dSocket.Send(bytes, nBytes, 0);
